...
- **Connected Knowledge — reading your graph** (epic #347): the committed knowledge graph becomes readable. A bounded, owner-scoped read API (#370: entity index and search, entity detail, a 1–2-hop neighbourhood capped at 300 nodes with `as_of`, a timeline that keeps superseded facts, mentions, citation links, and explorer expand — 404, never 403, for anything not yours); a precomputed whole-graph overview snapshot (#371: the `kg.graph_layout` job, Louvain clusters and ForceAtlas2 positions, labels always joined live); a cited entity brief with "since I last looked" (#372: deterministic sections plus a digest only the `kg.entity_digest` job ever writes — no AI call inside a request); and the `/graph` index and entity page (#373), reached from Home's "Knowledge" section, a named speaker's chip and library search, with every citation opening the exact transcript moment or note it came from; and a sigma.js graph explorer at `/graph/explore` plus a neighbourhood canvas on the entity page (#374: expand on click, server-side type filters, an `as_of` slider, a hard 300-node cap, and an accessible list view that doubles as the no-WebGL fallback); and a whole-graph overview at `/graph/overview` (#375: the precomputed cluster map drawn from #371's snapshot, with a cluster drill-down that opens that cluster's members in the explorer). See `docs/specs/ontology.md` §9, §12, §13 and §22.
...
- **`kvox deploy list`** (issue #290): the inventory of a server's apps root — one block per deployment with its name, deploy root, revision, bind port, domain, last deploy, and whether that came from the app's own state file or was inferred from the clone and the `.env`. `--json` for scripting, exit `2` when nothing is installed. Read from the filesystem alone: no container, no network and no `git` process, so it is instant with a dozen apps installed and answers the same when the Docker daemon is down. There is deliberately no central registry behind it — the registry is the apps root itself, one record per folder, colocated with what it describes. See `docs/specs/vps-deploy.md` §24.3.
- **Deploy-time application versioning** (issues #295, #296): `apps/api` and `apps/web` had both read `1.0.0` since the template, so the About page's Version fact never changed and two deployments of two different revisions were indistinguishable. `kvox deploy install`/`update` now choose a version — suggesting a patch bump, overridable at the prompt or with `--app-version <semver>`, skippable with `--no-version-bump` — write it to both `package.json` files and the root lockfile in lockstep (one shared product version), set `APP_VERSION` in the deployment's `.env` so the API reports it at runtime, and commit and push those files back to the deployed branch **only once the deployment is healthy and verified**. A push that cannot happen — a tag deploy, a fork with no push access, another server that pushed first — is a warning on the summary, never a failed deploy, and the bump commit is taken back out of the clone so the next update compares cleanly; it never force-pushes and never retries a rejected push, because the deployment was built from the *old* tip. An update with nothing to apply versions nothing, which is what keeps `deploy update` safe to run from cron. The number is also now visible to **every** user, not just administrators: `apps/web/package.json`'s version is baked into the bundle by a Vite `define` and shown as a quiet line at the foot of the shared settings hub — on `/settings` as well as `/admin/settings` — because `GET /api/admin/about` is gated on `system_settings:read`, seeded Admin-only. It reports what the *bundle* is rather than asking the API, so a stale cached bundle against a freshly deployed server shows up instead of being hidden. See `docs/specs/vps-deploy.md` §25 and `apps/cli/README.md`.
...
- **AI Name Correction for Transcripts** (epic #326, issues #327–#330): names and terms entered at upload are now forwarded to the transcription provider as a recognition hint (`keyterms`, up to 200 terms of up to 6 words each — AssemblyAI's `keyterms_prompt`; `GET /api/transcription/config` advertises `keytermsSupported`/`maxKeyterms` so the client can adapt to the active provider). A new "Fix names with AI" action on the transcript page — also offered right after renaming a speaker — runs deterministic phonetic candidate retrieval (Double Metaphone + Jaro-Winkler) followed by AI adjudication with an over-correction guard, on the requesting user's own AI provider key; an opt-in Thorough mode reads the whole transcript in chunks and shows a cost estimate before running. Suggestions are grouped for review ("Skar → Oscar ×12") and accepting them applies ordinary `segment.update_text` corrections as a single version, through the same `/operations` path any other edit uses. New endpoints under `/api/transcripts/{id}/name-checks*`, gated by `transcripts:read`/`transcripts:write` with the pipeline's usual 404-never-403 access model, backed by new `transcript_name_checks`/`transcript_name_suggestions` tables. See `docs/specs/transcript-name-correction.md`.
...
- **The deploy commands now know which app you are standing in** (issue #290): on a server hosting several applications under one apps root, `update`, `status`, `about`, `uninstall`, `doctor`, `certs` and every TUI deploy screen refused from inside the deployment's own directory — *"Several apps are installed under /opt/infra/apps: … Pass --name <app>"* — while standing in `/opt/infra/apps/<app>`. cwd is now a resolution rank of its own, below `--root`/`--name` and above both the sole-installed shortcut and that refusal, so no flag is needed from inside an app's folder however many the server hosts; it also finds a deployment whose state file has been lost, which the `install`-only rank it generalises could not. Enumeration additionally stops counting a neighbouring application that merely follows the same one-app-one-folder convention: an unrecorded deployment is recognised as this CLI's own by the `DEPLOY_ROOT` key `install` writes and `update` re-pins (or by the pre-`DEPLOY_ROOT` `.env` location), while `--root`/`--name` and the adoption path keep the looser evidence gate they had. See `docs/specs/vps-deploy.md` §24.
