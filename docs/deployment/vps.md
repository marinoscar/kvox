...
| `install`'s Google OAuth step fails on `google-oauth-credentials` | The client id/secret pair Google was asked about isn't real — most often a secret copied from a different, or since-rotated, OAuth client. | Re-copy both `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` from the *same* client's own page in Google Cloud console. If Google was simply unreachable (a restricted network), this step **warns** rather than fails, and does not block the install. |
