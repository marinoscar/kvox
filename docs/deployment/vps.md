...
2. **Run `install`:**
   ...
   | Resources | The loopback port, job worker slots, and container memory limits — each **measured from this server and applied, not asked** (issue #257), shown with its reason here and in the Review step. Pass `--answer APP_BIND_PORT=3600` (or any of the four) to decide one yourself, or `--all` to be asked about every one; a value already in `.env` is never second-guessed. The port clears three sources: the other apps under `--apps-root`, every host port Docker has been told to publish (including **stopped** containers, which nothing else can see), and a live bind probe | — |
...
| `install` stops at the **Start the stack** step saying the port "was free when it was chosen and is not free now" | Something took the port during the build/migrate/seed window — minutes, in a real install. The message names the container where Docker can say. | Free the port, or re-run with `--answer APP_BIND_PORT=<n>` to publish on another one. The port is **not** re-picked automatically on purpose: an external proxy or a DNS record may already point at the one you were given. This deployment's own containers are never treated as a collision, so `--resume` after a failed health step is unaffected. |
