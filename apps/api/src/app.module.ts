...
@Module({
  imports: [
    ...
    // Ask (#376, epic #348): saved conversations with the read-only graph
    // agent. Imports GraphModule for `GraphAccessService` (scope validation);
    // imported by nothing, so the edge runs one way.
    AskModule,
