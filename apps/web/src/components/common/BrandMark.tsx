 ...
 *
 * The K monogram — one vertical stem and two round-capped diagonal arms — is
 * already described twice: in `apps/web/public/favicon.svg` and, because this
 * template refuses to make a fork's CI install an SVG rasteriser, a second
 * time as Pillow constants in `apps/web/scripts/generate-icons.py`. A
 * component that inlined a third copy as JSX would be a third thing to keep in
 * step, and the one with the fewest guards: the two files above are compared
 * shape for shape by `src/__tests__/pwa/brandMark.test.ts`, and a JSX copy
 * would sit outside that comparison and drift silently. So this renders the
 * committed file. The favicon crop (mark box at 80% of the canvas) is also the
 * right one here — this is a ~28px mark, the same size regime the tab icon
 * lives in.
 *
