...
import { PSEUDO_TYPES } from './constants.js';
...
import type { AttributeSpec, DomainModule, EntityTypeSpec, OntologyRegistry, RelationTypeSpec } from './types.js';
...
function assertTargetTypes(owner: string, attributes: Record<string, AttributeSpec>, entityKeys: Set<string>): void {
  for (const [key, spec] of Object.entries(attributes)) {
    for (const target of spec.options?.targetTypes ?? []) {
      if (!entityKeys.has(target)) fail(`${owner}.${key}: entity_ref target type '${target}' is not a registered entity type`);
    }
  }
}
...
export function buildOntologyRegistry(mods: DomainModule[], version: string): OntologyRegistry {
  if (!SEMVER_PATTERN.test(version)) fail(`Ontology version '${version}' is not semver (MAJOR.MINOR.PATCH)`);
  ...
  for (const mod of mods) {
    if (domainKeys.has(mod.key)) fail(`Duplicate domain module '${mod.key}'`);
    domainKeys.add(mod.key);
    for (const type of mod.entityTypes) {
      if (type.domain !== mod.key) fail(`${type.key}: listed in domain '${mod.key}' but declares domain '${type.domain}'`);
      if (entities.has(type.key)) fail(`Duplicate entity type key '${type.key}'`);
      entities.set(type.key, type);
    }
    for (const rel of mod.relationTypes) {
      if (rel.domain !== mod.key) fail(`${rel.key}: listed in domain '${mod.key}' but declares domain '${rel.domain}'`);
      if (relations.has(rel.key)) fail(`Duplicate relation type key '${rel.key}'`);
      relations.set(rel.key, rel);
    }
  }

  const entityKeys = new Set(entities.keys());
  const pseudo = new Set<string>(PSEUDO_TYPES);
  for (const key of entityKeys) {
    if (pseudo.has(key)) fail(`${key}: entity type key collides with a pseudo-type`);
  }

  for (const type of entities.values()) {
    for (const subject of type.subjectTypes ?? []) {
      if (!entityKeys.has(subject)) fail(`${type.key}: subject type '${subject}' is not a registered entity type`);
    }
  ...
  for (const type of entities.values()) {
    for (const key of Object.keys(type.attributes)) attributeOwners.set(`${type.key}.${key}`, `base ${type.key}`);
  }
  for (const mod of mods) {
    for (const mixin of mod.mixins) {
      if (!entityKeys.has(mixin.entityType)) fail(`Mixin from '${mod.key}' targets unknown entity type '${mixin.entityType}'`);
      for (const key of Object.keys(mixin.attributes)) {
        const name = `${mixin.entityType}.${key}`;
        const existing = attributeOwners.get(name);
        if (existing !== undefined) fail(`${name}: mixin from '${mod.key}' collides with an attribute declared by ${existing}`);
  ...
  const domainList = deepFreeze([...mods]);
