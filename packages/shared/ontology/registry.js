...
exports.SEMVER_PATTERN = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/;
...
function buildOntologyRegistry(mods, version) {
    if (typeof version !== 'string' || !exports.SEMVER_PATTERN.test(version)) {
        fail('ontology', `version '${String(version)}' is not semver MAJOR.MINOR.PATCH`);
    }
    const domains = mods.map((m) => (0, define_js_1.defineDomain)(m));
    const domainKeys = new Set();
    for (const d of domains) {
        if (!constants_js_1.DOMAIN_KEYS.includes(d.key))
            fail(`domain ${d.key}`, 'unknown domain key');
        if (domainKeys.has(d.key))
            fail(`domain ${d.key}`, 'declared twice');
        domainKeys.add(d.key);
    }
    if (!domainKeys.has('core'))
        fail('ontology', "the 'core' domain is required");
    for (const d of domains) {
        if (d.alwaysOn && d.key !== 'core')
            fail(`domain ${d.key}`, "only 'core' may be always on");
    }
    const entities = new Map();
    const relations = new Map();
    for (const d of domains) {
        for (const t of d.entityTypes) {
            if (entities.has(t.key))
                fail(`entity type ${t.key}`, 'declared twice');
            if (constants_js_1.PSEUDO_TYPES.includes(t.key))
                fail(`entity type ${t.key}`, 'collides with a pseudo-type');
            entities.set(t.key, t);
        }
        for (const r of d.relationTypes) {
            if (relations.has(r.key))
                fail(`relation type ${r.key}`, 'declared twice');
            relations.set(r.key, r);
        }
    }
    const isEntityStorage = (key) => {
    ...
    for (const t of entities.values()) {
        if (t.itemKind === undefined)
            continue;
        if (itemKinds.has(t.itemKind))
            fail(`entity type ${t.key}`, `itemKind '${t.itemKind}' is already used`);
        itemKinds.add(t.itemKind);
        for (const s of t.subjectTypes ?? []) {
            if (!isEntityStorage(s))
                fail(`entity type ${t.key}`, `subject type '${s}' is not a registered entity-storage type`);
        }
    }
    for (const k of itemKinds) {
        if (!constants_js_1.ITEM_KINDS.includes(k))
            fail('ontology', `unknown itemKind '${k}'`);
    }
    ...
    for (const r of relations.values()) {
        for (const side of ['from', 'to']) {
            for (const endpoint of r[side]) {
                const pseudo = constants_js_1.PSEUDO_TYPES.includes(endpoint);
