...
exports.ATTRIBUTE_KEY_PATTERN = exports.RELATION_TYPE_KEY_PATTERN = exports.ENTITY_TYPE_KEY_PATTERN = exports.OntologyDefinitionError = void 0;
exports.deepFreeze = deepFreeze;
exports.assertAttributeSpec = assertAttributeSpec;
 ...
 */
function assertAttributeSpec(owner, key, spec) {
    const name = `${owner}.${key}`;
    if (key.startsWith(constants_js_1.USER_ATTRIBUTE_KEY_PREFIX)) {
        fail(`${name}: attribute keys starting '${constants_js_1.USER_ATTRIBUTE_KEY_PREFIX}' are reserved for user-defined attributes`);
    }
    if (!exports.ATTRIBUTE_KEY_PATTERN.test(key)) {
        fail(`${name}: attribute key must match ${String(exports.ATTRIBUTE_KEY_PATTERN)} (camelCase)`);
    }
    if (spec === null || typeof spec !== 'object') {
        fail(`${name}: attribute declaration must be an object`);
    }
    if (!includes(constants_js_1.ATTRIBUTE_KINDS, spec.kind)) {
        fail(`${name}: kind must be one of ${constants_js_1.ATTRIBUTE_KINDS.join(', ')} (got ${String(spec.kind)})`);
    }
    if (!isNonEmptyString(spec.label))
        fail(`${name}: label must not be empty`);
    if (!isNonEmptyString(spec.description))
        fail(`${name}: description must not be empty`);
    if (spec.sensitivity !== undefined && !includes(constants_js_1.SENSITIVITIES, spec.sensitivity)) {
        fail(`${name}: sensitivity must be one of ${constants_js_1.SENSITIVITIES.join(', ')}`);
    }
    if (spec.kind === 'select' || spec.kind === 'multi_select') {
        const choices = spec.options?.choices ?? [];
        if (choices.length === 0)
            fail(`${name}: a ${spec.kind} attribute needs at least one choice`);
        const seen = new Set();
        for (const choice of choices) {
            if (!isNonEmptyString(choice.value))
                fail(`${name}: every choice needs a non-empty value`);
            if (!isNonEmptyString(choice.label))
                fail(`${name}: choice '${choice.value}' needs a non-empty label`);
            if (seen.has(choice.value))
                fail(`${name}: duplicate choice value '${choice.value}'`);
    ...
    if (spec.options?.targetTypes !== undefined) {
        if (spec.kind !== 'entity_ref')
            fail(`${name}: only entity_ref attributes take targetTypes`);
        if (spec.options.targetTypes.length === 0)
            fail(`${name}: targetTypes, when given, must not be empty`);
    }
...
function assertAttributes(owner, attributes) {
...
function assertRepresentation(key, representation) {
...
function defineEntityType(spec) {
    const key = spec?.key;
    if (!isNonEmptyString(key) || !exports.ENTITY_TYPE_KEY_PATTERN.test(key)) {
        fail(`Entity type '${String(key)}': key must match ${String(exports.ENTITY_TYPE_KEY_PATTERN)} (PascalCase)`);
    }
    if (!includes(constants_js_1.DOMAIN_KEYS, spec.domain))
        fail(`${key}: domain must be one of ${constants_js_1.DOMAIN_KEYS.join(', ')}`);
    if (!isNonEmptyString(spec.label))
        fail(`${key}: label must not be empty`);
    if (!isNonEmptyString(spec.pluralLabel))
        fail(`${key}: pluralLabel must not be empty`);
    if (!isNonEmptyString(spec.description))
        fail(`${key}: description must not be empty`);
    if (!Array.isArray(spec.disambiguation) || spec.disambiguation.length === 0) {
        fail(`${key}: needs at least one disambiguation rule`);
    }
    if (!spec.disambiguation.every(isNonEmptyString))
        fail(`${key}: disambiguation rules must not be empty`);
    if (!includes(constants_js_1.SENSITIVITIES, spec.sensitivityDefault)) {
        fail(`${key}: sensitivityDefault must be one of ${constants_js_1.SENSITIVITIES.join(', ')}`);
    }
    assertAttributes(key, spec.attributes);
    if (spec.itemKind !== undefined) {
        if (!includes(constants_js_1.ITEM_KINDS, spec.itemKind))
            fail(`${key}: itemKind must be one of ${constants_js_1.ITEM_KINDS.join(', ')}`);
        if (!Array.isArray(spec.statuses) || spec.statuses.length === 0)
            fail(`${key}: an item type needs statuses`);
        if (new Set(spec.statuses).size !== spec.statuses.length)
            fail(`${key}: duplicate status`);
        if (!Array.isArray(spec.subjectTypes) || spec.subjectTypes.length === 0) {
            fail(`${key}: an item type needs subjectTypes`);
        }
        if (typeof spec.subjectRequired !== 'boolean')
            fail(`${key}: an item type must state subjectRequired`);
    }
...
function defineRelationType(spec) {
    const key = spec?.key;
    if (!isNonEmptyString(key) || !exports.RELATION_TYPE_KEY_PATTERN.test(key)) {
        fail(`Relation type '${String(key)}': key must match ${String(exports.RELATION_TYPE_KEY_PATTERN)} (SCREAMING_SNAKE)`);
    }
    if (!includes(constants_js_1.DOMAIN_KEYS, spec.domain))
        fail(`${key}: domain must be one of ${constants_js_1.DOMAIN_KEYS.join(', ')}`);
    if (!isNonEmptyString(spec.label))
    ...
    if (!Array.isArray(spec.from) || spec.from.length === 0)
        fail(`${key}: from must name at least one endpoint type`);
    if (!Array.isArray(spec.to) || spec.to.length === 0)
        fail(`${key}: to must name at least one endpoint type`);
    ...
    if (spec.exclusive !== 'soft' && spec.exclusive !== 'none')
        fail(`${key}: exclusive must be 'soft' or 'none'`);
    ...
    if (spec.allowedPairs !== undefined) {
        if (spec.allowedPairs.length === 0)
            fail(`${key}: allowedPairs, when given, must not be empty`);
...
function defineDomain(mod) {
    const key = mod?.key;
    if (!includes(constants_js_1.DOMAIN_KEYS, key))
        fail(`Domain '${String(key)}': key must be one of ${constants_js_1.DOMAIN_KEYS.join(', ')}`);
    if (!isNonEmptyString(mod.label))
        fail(`Domain ${key}: label must not be empty`);
    if (mod.alwaysOn && !mod.defaultEnabled)
        fail(`Domain ${key}: an always-on domain must also be default-enabled`);
    const seen = new Set();
    for (const type of [...mod.entityTypes, ...mod.relationTypes]) {
        if (type.domain !== key)
            fail(`${type.key}: declared in domain module '${key}' but says domain '${type.domain}'`);
        if (seen.has(type.key))
            fail(`Domain ${key}: duplicate key '${type.key}'`);
        seen.add(type.key);
    }
    for (const mixin of mod.mixins) {
        if (!isNonEmptyString(mixin.entityType))
            fail(`Domain ${key}: a mixin needs an entityType`);
